...
import {
  AverageExperienceDepartmentData,
  createAverageExperienceDepartment,
  deleteAverageExperienceDepartment,
  getAverageExperienceDepartment,
  getAverageExperienceDepartmentLatest,
  updateAverageExperienceDepartment,
} from "../../../api";
...
const AverageExperienceDepartmentTable = () => {
  ...
  const handleCreateAverageExperienceDepartment = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createAverageExperienceDepartment({
        name: statsName,
        value: statsValue,
      ...
      console.log("createAverageExperienceDepartment error:", error);
    }
  };
  ...
  const handleGetAverageExperienceDepartment = async (
    ...
    try {
      ...
      console.log("getAverageExperienceDepartment error:", error);
    }
  };
  ...
  const handleDeleteAverageExperienceDepartment = async (id: number) => {
    try {
      await deleteAverageExperienceDepartment(id);
      handleGetAverageExperienceDepartment(filteredMonth);
    } catch (error) {
      console.log("deleteAverageExperienceDepartment error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    if (
      ...
      return;
    try {
      await updateAverageExperienceDepartment({
        id: updateStatsId,
      ...
      console.log("updateAverageExperienceDepartment error:", error);
    }
  };

  const handleUpdateClick = (x: AverageExperienceDepartmentData) => {
    ...
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!averageExperienceDepartment &&
          !!(averageExperienceDepartment.length > 0) &&
          averageExperienceDepartment.map((x, idx) => (
            <tr key={"averageExperienceDepartment-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteAverageExperienceDepartment(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateAverageExperienceDepartment}
            >
...
export default AverageExperienceDepartmentTable;
