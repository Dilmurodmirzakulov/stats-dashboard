...
import {
  YearsWithUniversityData,
  createYearsWithUniversity,
  deleteYearsWithUniversity,
  getYearsWithUniversity,
  getYearsWithUniversityLatest,
  updateYearsWithUniversity,
} from "../../../api";
...
const YearsWithUniversityTable = () => {
  ...
  const handleCreateYearsWithUniversity = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createYearsWithUniversity({
        name: statsName,
        value: statsValue,
      ...
      console.log("createYearsWithUniversity error:", error);
    }
  };
  ...
  const handleGetYearsWithUniversity = async (filteredMonth: string = "") => {
    try {
      ...
      console.log("getYearsWithUniversity error:", error);
    }
  };
  ...
  const handleDeleteYearsWithUniversity = async (id: number) => {
    try {
      await deleteYearsWithUniversity(id);
      handleGetYearsWithUniversity(filteredMonth);
    } catch (error) {
      console.log("deleteYearsWithUniversity error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    if (
      ...
      return;
    try {
      await updateYearsWithUniversity({
        id: updateStatsId,
      ...
      console.log("updateYearsWithUniversity error:", error);
    }
  };

  const handleUpdateClick = (x: YearsWithUniversityData) => {
  ...
  return (
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!yearsWithUniversity &&
          !!(yearsWithUniversity.length > 0) &&
          yearsWithUniversity.map((x, idx) => (
            <tr key={"yearsWithUniversity-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteYearsWithUniversity(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateYearsWithUniversity}
            >
...
export default YearsWithUniversityTable;
