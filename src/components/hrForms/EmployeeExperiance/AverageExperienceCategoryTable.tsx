...
import {
  AverageExperienceCategoryData,
  createAverageExperienceCategory,
  deleteAverageExperienceCategory,
  getAverageExperienceCategory,
  getAverageExperienceCategoryLatest,
  updateAverageExperienceCategory,
} from "../../../api";
...
const AverageExperienceCategoryTable = () => {
  ...
  const handleCreateAverageExperienceCategory = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createAverageExperienceCategory({
        name: statsName,
        value: statsValue,
      ...
      setStatsDate(null);
      setStatsValue(null);
      handleGetAverageExperienceCategory();
    } catch (error) {
      console.log("createAverageExperienceCategory error:", error);
    }
  };
  ...
  const handleGetAverageExperienceCategory = async (
    ...
    try {
      ...
      console.log("getAverageExperienceCategory error:", error);
    }
  };
  ...
  const handleDeleteAverageExperienceCategory = async (id: number) => {
    try {
      await deleteAverageExperienceCategory(id);
      handleGetAverageExperienceCategory(filteredMonth);
    } catch (error) {
      console.log("deleteAverageExperienceCategory error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    if (
      ...
      return;
    try {
      await updateAverageExperienceCategory({
        id: updateStatsId,
      ...
      console.log("updateAverageExperienceCategory error:", error);
    }
  };

  const handleUpdateClick = (x: AverageExperienceCategoryData) => {
    ...
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!averageExperienceCategory &&
          !!(averageExperienceCategory.length > 0) &&
          averageExperienceCategory.map((x, idx) => (
            <tr key={"averageExperienceCategory-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteAverageExperienceCategory(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateAverageExperienceCategory}
            >
...
export default AverageExperienceCategoryTable;
