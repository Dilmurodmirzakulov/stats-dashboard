...
import {
  TerminationMonthsData,
  createTerminationMonths,
  deleteTerminationMonths,
  getTerminationMonths,
  getTerminationMonthsLatest,
  updateTerminationMonths,
} from "../../../api";
...
const TerminationMonthsTable = () => {
  ...
  const handleCreateTerminationMonths = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createTerminationMonths({
        name: statsName,
        value: statsValue,
      ...
      console.log("createTerminationMonths error:", error);
    }
  };
  ...
  const handleGetTerminationMonths = async (filteredMonth: string = "") => {
    try {
      ...
      console.log("getTerminationMonths error:", error);
    }
  };
  ...
  const handleDeleteTerminationMonths = async (id: number) => {
    try {
      await deleteTerminationMonths(id);
      handleGetTerminationMonths(filteredMonth);
    } catch (error) {
      console.log("deleteTerminationMonths error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    ...
    try {
      ...
      console.log("updateTerminationMonths error:", error);
    }
  };

  const handleUpdateClick = (x: TerminationMonthsData) => {
  ...
  return (
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!terminationMonths &&
          !!(terminationMonths.length > 0) &&
          terminationMonths.map((x, idx) => (
            <tr key={"terminationMonths-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteTerminationMonths(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateTerminationMonths}
            >
...
export default TerminationMonthsTable;
