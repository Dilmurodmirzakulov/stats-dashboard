import React, { useEffect, useState } from "react";
import { Table } from "react-bootstrap";
import {
  TopTenSignedPersonelData,
  createTopTenSignedPersonel,
  deleteTopTenSignedPersonel,
  getTopTenSignedPersonel,
  getTopTenSignedPersonelLatest,
  updateTopTenSignedPersonel,
} from "../../../api";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "../../../store";
import { setTopTenSignedPersonel } from "../../../store/slices/DMS/topTenSignedPersonelSlice";
import { formatDate } from "../../../utils/custom";

const TopTenSignedPersonelTable = () => {
  const dispatch = useDispatch();
  const { topTenSignedPersonel } = useSelector(
    (state: RootState) => state.topTenSignedPersonelReducer
  );
  const [filteredMonth, setFilteredMonth] = useState("");

  const [statsName, setStatsName] = useState<null | string>(null);
  const [statsValue, setStatsValue] = useState<null | number>(null);
  const [statsTime, setStatsTime] = useState<null | number>(null);
  const [statsDate, setStatsDate] = useState<null | string>(null);

  const [updateStatsId, setUpdateStatsId] = useState<null | number>(null);
  const [updateStatsName, setUpdateStatsName] = useState<null | string>(null);
  const [updateStatsValue, setUpdateStatsValue] = useState<null | number>(null);
  const [updateStatsTime, setUpdateStatsTime] = useState<null | number>(null);
  const [updateStatsDate, setUpdateStatsDate] = useState<null | string>(null);

  const currentMonth = new Date().getMonth();
  const currentYear = new Date().getFullYear();

  const handleCreateTopTenSignedPersonel = async () => {
    if (!statsName || !statsValue || !statsDate || !statsTime) return;
    try {
      await createTopTenSignedPersonel({
        name: statsName,
        value: statsValue,
        time: statsTime,
        calculated_date: statsDate,
      });
      setStatsName(null);
      setStatsDate(null);
      setStatsValue(null);
      setStatsTime(null);
      handleGetTopTenSignedPersonel(filteredMonth);
    } catch (error) {
      console.log("createTopTenSignedPersonel error:", error);
    }
  };

  const handleGetTopTenSignedPersonel = async (filteredMonth: string = "") => {
    try {
      let res = await getTopTenSignedPersonel(filteredMonth);
      dispatch(setTopTenSignedPersonel(res.data));
    } catch (error) {
      console.log("getTopTenSignedPersonel error:", error);
    }
  };

  const handleGetLastReports = async () => {
    try {
      const res = await getTopTenSignedPersonelLatest();
      if (res.data && res.data.length > 0)
        return setFilteredMonth(res.data[0].calculated_date);
      setFilteredMonth("");
    } catch (error) {
      console.log("error getTopTenSignedPersonelLatest: ", error);
    }
  };

  useEffect(() => {
    handleGetLastReports();
  }, []);

  const handleDeleteTopTenSignedPersonel = async (id: number) => {
    try {
      await deleteTopTenSignedPersonel(id);
      handleGetTopTenSignedPersonel(filteredMonth);
    } catch (error) {
      console.log("deleteTopTenSignedPersonel error:", error);
    }
  };

  const handleUpdateStats = async () => {
    if (
      !updateStatsId ||
      !updateStatsDate ||
      !updateStatsValue ||
      !updateStatsTime ||
      !updateStatsName
    )
      return;
    try {
      await updateTopTenSignedPersonel({
        id: updateStatsId,
        name: updateStatsName,
        value: updateStatsValue,
        time: updateStatsTime,
        calculated_date: updateStatsDate,
      });
      setUpdateStatsId(null);
      setUpdateStatsName(null);
      setUpdateStatsValue(null);
      setUpdateStatsTime(null);
      setUpdateStatsDate(null);
      handleGetTopTenSignedPersonel(filteredMonth);
    } catch (error) {
      console.log("updateTopTenSignedPersonel error:", error);
    }
  };

  const handleUpdateClick = (x: TopTenSignedPersonelData) => {
    setUpdateStatsName(x.name);
    setUpdateStatsValue(x.value);
    setUpdateStatsTime(x.time);
    setUpdateStatsDate(x.calculated_date);
    setUpdateStatsId(x.id);
  };

  useEffect(() => {
    handleGetTopTenSignedPersonel(filteredMonth);
  }, [filteredMonth]);

  return (
    <Table striped hover responsive className="mb-3">
      <thead>
        <tr>
          <th>#</th>
          <th>Name</th>
          <th>Value</th>
          <th>Time</th>
          <th>Report month</th>
          <th>Created at</th>
          <th>&nbsp;</th>
        </tr>
        <tr>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td>
            <input
              onChange={(e) => setFilteredMonth(e.target.value)}
              value={filteredMonth}
              className="form-control"
              type="month"
            />
          </td>
          <td></td>
          <td></td>
        </tr>
      </thead>
      <tbody>
        {!!topTenSignedPersonel &&
          !!(topTenSignedPersonel.length > 0) &&
          topTenSignedPersonel.map((x, idx) => (
            <tr key={"topTenSignedPersonel-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
                ) : (
                  <input
                    type="text"
                    className="form-control"
                    value={updateStatsName || ""}
                    onChange={(e) => setUpdateStatsName(e.target.value)}
                  />
                )}
              </td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.value}</span>
                ) : (
                  <input
                    type="number"
                    step="any"
                    className="form-control"
                    value={updateStatsValue || ""}
                    onChange={(e) =>
                      setUpdateStatsValue(parseFloat(e.target.value))
                    }
                  />
                )}
              </td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.time}</span>
                ) : (
                  <input
                    type="text"
                    className="form-control"
                    value={updateStatsTime || ""}
                    onChange={(e) =>
                      setUpdateStatsTime(parseInt(e.target.value, 10))
                    }
                  />
                )}
              </td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.calculated_date}</span>
                ) : (
                  <input
                    type="month"
                    className="form-control"
                    value={updateStatsDate || ""}
                    onChange={(e) => setUpdateStatsDate(e.target.value)}
                  />
                )}
              </td>
              <td>
                <span>{formatDate(x.created_at)}</span>
              </td>
              <td className="text-end">
                {!(updateStatsId == x.id) ? (
                  <button
                    className="btn p-1"
                    onClick={() => handleUpdateClick(x)}
                  >
                    <i className="bx bxs-pencil"></i>
                  </button>
                ) : (
                  <button className="btn p-1" onClick={handleUpdateStats}>
                    <i className="bx bx-check"></i>
                  </button>
                )}
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteTopTenSignedPersonel(x.id)}
                >
                  <i className="bx bxs-trash"></i>
                </button>
              </td>
            </tr>
          ))}
        <tr>
          <td> </td>
          <td>
            <input
              type="text"
              className="form-control"
              onChange={(e) => setStatsName(e.target.value)}
              value={statsName || ""}
            />
          </td>
          <td>
            <input
              type="number"
              step="any"
              className="form-control"
              onChange={(e) => setStatsValue(parseFloat(e.target.value))}
              value={statsValue || ""}
            />
          </td>
          <td>
            <input
              type="text"
              className="form-control"
              onChange={(e) => setStatsTime(parseInt(e.target.value, 10))}
              value={statsTime || ""}
            />
          </td>
          <td>
            <input
              type="month"
              onChange={(e) => setStatsDate(e.target.value)}
              min={`${currentYear}-${currentMonth < 9 ? "0" : ""}${
                currentMonth + 1
              }`}
              className="form-control"
              value={statsDate || ""}
            />
          </td>
          <td></td>
          <td className="text-end">
            <button
              className="btn p-1 btn-icon btn-success"
              onClick={handleCreateTopTenSignedPersonel}
            >
              <i className="bx bx-plus"></i>
            </button>
          </td>
        </tr>
      </tbody>
    </Table>
  );
};

export default TopTenSignedPersonelTable;
