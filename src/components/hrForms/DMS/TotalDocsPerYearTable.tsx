...
import {
  TotalDocsPerYearData,
  createTotalDocsPerYear,
  deleteTotalDocsPerYear,
  getTotalDocsPerYear,
  getTotalDocsPerYearLatest,
  updateTotalDocsPerYear,
} from "../../../api";
...
const TotalDocsPerYearTable = () => {
  ...
  const handleCreateTotalDocsPerYear = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createTotalDocsPerYear({
        name: statsName,
        value: statsValue,
      ...
      console.log("createTotalDocsPerYear error:", error);
    }
  };
  ...
  const handleGetTotalDocsPerYear = async (filteredMonth: string = "") => {
    try {
      ...
      console.log("getTotalDocsPerYear error:", error);
    }
  };
  ...
  const handleDeleteTotalDocsPerYear = async (id: number) => {
    try {
      await deleteTotalDocsPerYear(id);
      handleGetTotalDocsPerYear(filteredMonth);
    } catch (error) {
      console.log("deleteTotalDocsPerYear error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    ...
    try {
      ...
      console.log("updateTotalDocsPerYear error:", error);
    }
  };

  const handleUpdateClick = (x: TotalDocsPerYearData) => {
  ...
  return (
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!totalDocsPerYear &&
          !!(totalDocsPerYear.length > 0) &&
          totalDocsPerYear.map((x, idx) => (
            <tr key={"totalDocsPerYear-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteTotalDocsPerYear(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateTotalDocsPerYear}
            >
...
export default TotalDocsPerYearTable;
