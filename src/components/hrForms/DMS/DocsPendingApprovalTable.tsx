...
import {
  DocsPendingApprovalData,
  createDocsPendingApproval,
  deleteDocsPendingApproval,
  getDocsPendingApproval,
  getDocsPendingApprovalLatest,
  updateDocsPendingApproval,
} from "../../../api";
...
const DocsPendingApprovalTable = () => {
  ...
  const handleCreateDocsPendingApproval = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createDocsPendingApproval({
        name: statsName,
        value: statsValue,
      ...
      console.log("createDocsPendingApproval error:", error);
    }
  };
  ...
  const handleGetDocsPendingApproval = async (filteredMonth: string = "") => {
    try {
      ...
      console.log("getDocsPendingApproval error:", error);
    }
  };
  ...
  const handleDeleteDocsPendingApproval = async (id: number) => {
    try {
      await deleteDocsPendingApproval(id);
      handleGetDocsPendingApproval(filteredMonth);
    } catch (error) {
      console.log("deleteDocsPendingApproval error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    if (
      ...
      return;
    try {
      await updateDocsPendingApproval({
        id: updateStatsId,
      ...
      console.log("updateDocsPendingApproval error:", error);
    }
  };

  const handleUpdateClick = (x: DocsPendingApprovalData) => {
  ...
  return (
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!docsPendingApproval &&
          !!(docsPendingApproval.length > 0) &&
          docsPendingApproval.map((x, idx) => (
            <tr key={"docsPendingApproval-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteDocsPendingApproval(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateDocsPendingApproval}
            >
...
export default DocsPendingApprovalTable;
