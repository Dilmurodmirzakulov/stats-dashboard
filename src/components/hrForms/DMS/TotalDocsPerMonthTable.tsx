...
import {
  TotalDocsPerMonthData,
  createTotalDocsPerMonth,
  deleteTotalDocsPerMonth,
  getTotalDocsPerMonth,
  getTotalDocsPerMonthLatest,
  updateTotalDocsPerMonth,
} from "../../../api";
...
const TotalDocsPerMonthTable = () => {
  ...
  const handleCreateTotalDocsPerMonth = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createTotalDocsPerMonth({
        name: statsName,
        value: statsValue,
      ...
      console.log("createTotalDocsPerMonth error:", error);
    }
  };
  ...
  const handleGetTotalDocsPerMonth = async (filteredMonth: string = "") => {
    try {
      ...
      console.log("getTotalDocsPerMonth error:", error);
    }
  };
  ...
  const handleDeleteTotalDocsPerMonth = async (id: number) => {
    try {
      await deleteTotalDocsPerMonth(id);
      handleGetTotalDocsPerMonth(filteredMonth);
    } catch (error) {
      console.log("deleteTotalDocsPerMonth error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    ...
    try {
      ...
      console.log("updateTotalDocsPerMonth error:", error);
    }
  };

  const handleUpdateClick = (x: TotalDocsPerMonthData) => {
  ...
  return (
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!totalDocsPerMonth &&
          !!(totalDocsPerMonth.length > 0) &&
          totalDocsPerMonth.map((x, idx) => (
            <tr key={"totalDocsPerMonth-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteTotalDocsPerMonth(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateTotalDocsPerMonth}
            >
...
export default TotalDocsPerMonthTable;
