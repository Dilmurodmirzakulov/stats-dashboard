...
import {
  DocsStatusDistData,
  createDocsStatusDist,
  deleteDocsStatusDist,
  getDocsStatusDist,
  getDocsStatusDistLatest,
  updateDocsStatusDist,
} from "../../../api";
...
const DocsStatusDistTable = () => {
  ...
  const handleCreateDocsStatusDist = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createDocsStatusDist({
        name: statsName,
        value: statsValue,
      ...
      console.log("createDocsStatusDist error:", error);
    }
  };
  ...
  const handleGetDocsStatusDist = async (filteredMonth: string = "") => {
    try {
      ...
      console.log("getDocsStatusDist error:", error);
    }
  };
  ...
  const handleDeleteDocsStatusDist = async (id: number) => {
    try {
      await deleteDocsStatusDist(id);
      handleGetDocsStatusDist(filteredMonth);
    } catch (error) {
      console.log("deleteDocsStatusDist error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    ...
    try {
      ...
      console.log("updateDocsStatusDist error:", error);
    }
  };

  const handleUpdateClick = (x: DocsStatusDistData) => {
          ...
          docsStatusDist.map((x, idx) => (
            <tr key={"docsStatusDist-id-" + x.id}>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteDocsStatusDist(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateDocsStatusDist}
            >
...
export default DocsStatusDistTable;
