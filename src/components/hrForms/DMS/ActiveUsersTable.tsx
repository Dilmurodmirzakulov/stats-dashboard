...
const ActiveUsersTable = () => {
  ...
  const handleCreateActiveUsers = async () => {
    ...
    try {
      ...
      console.log("createActiveUsers error:", error);
    }
  };
  ...
  const handleDeleteActiveUser = async (id: number) => {
    try {
      await deleteActiveUser(id);
      handleGetActiveUsers(filteredMonth);
    } catch (error) {
      console.log("deleteActiveUser error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    ...
    try {
      ...
      console.log("updateActiveUser error:", error);
    }
  };

  const handleUpdateClick = (x: ActiveUserData) => {
    ...
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!activeUsers &&
          ...
          activeUsers.map((x, idx) => (
            <tr key={"activeUsers-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteActiveUser(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateActiveUsers}
            >
