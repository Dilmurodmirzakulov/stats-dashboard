...
const OffersTable = () => {
  ...
  const handleCreateOffers = async () => {
    ...
    try {
      ...
      console.log("createOffers error:", error);
    }
  };
  ...
  const handleDeleteOffers = async (id: number) => {
    try {
      await deleteOffers(id);
      handleGetOffers(filteredMonth);
    } catch (error) {
      console.log("deleteOffers error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    ...
    try {
      ...
      console.log("updateOffers error:", error);
    }
  };

  const handleUpdateClick = (x: OffersData) => {
  ...
  return (
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!offers &&
          !!(offers.length > 0) &&
          offers.map((x: OffersData, idx: number) => (
            <tr key={"offers-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteOffers(x.id)}
                >
