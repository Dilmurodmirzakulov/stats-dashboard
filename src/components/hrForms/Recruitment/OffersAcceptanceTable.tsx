...
import {
  OffersAcceptanceData,
  createOffersAcceptance,
  deleteOffersAcceptance,
  getOffersAcceptance,
  getOffersAcceptanceLatest,
  updateOffersAcceptance,
} from "../../../api";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "../../../store";
import { setOffersAcceptance } from "../../../store/slices/Recruitment/offersAcceptanceSlice";
import { formatDate } from "../../../utils/custom";

const OffersAcceptanceTable = () => {
  const dispatch = useDispatch();
  const { offersAcceptance } = useSelector(
    (state: RootState) => state.offersAcceptanceReducer
  );
  ...
  const handleCreateOffersAcceptance = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createOffersAcceptance({
        name: statsName,
        value: statsValue,
      ...
      setStatsDate(null);
      setStatsValue(null);
      handleGetOffersAcceptance(filteredMonth);
    } catch (error) {
      console.log("createOffersAcceptance error:", error);
    }
  };

  const handleGetOffersAcceptance = async (filteredMonth: string = "") => {
    try {
      let res = await getOffersAcceptance(filteredMonth);
      dispatch(setOffersAcceptance(res.data));
    } catch (error) {
      console.log("getOffersAcceptance error:", error);
    }
  };

  const handleGetLastReports = async () => {
    try {
      const res = await getOffersAcceptanceLatest();
      if (res.data && res.data.length > 0)
        return setFilteredMonth(res.data[0].calculated_date);
      setFilteredMonth("");
    } catch (error) {
      console.log("error getOffersAcceptanceLatest: ", error);
    }
  };
  ...
  const handleDeleteOffersAcceptance = async (id: number) => {
    try {
      await deleteOffersAcceptance(id);
      handleGetOffersAcceptance(filteredMonth);
    } catch (error) {
      console.log("deleteOffersAcceptance error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    ...
    try {
      ...
      console.log("updateOffersAcceptance error:", error);
    }
  };

  const handleUpdateClick = (x: OffersAcceptanceData) => {
  ...
  return (
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!offersAcceptance &&
          !!(offersAcceptance.length > 0) &&
          offersAcceptance.map((x: OffersAcceptanceData, idx: number) => (
            <tr key={"offersAcceptance-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteOffersAcceptance(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateOffersAcceptance}
            >
...
export default OffersAcceptanceTable;
