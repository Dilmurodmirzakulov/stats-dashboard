...
import {
  NumberOfInquiriesByTypeTrendData,
  createNumberOfInquiriesByTypeTrend,
  deleteNumberOfInquiriesByTypeTrend,
  getNumberOfInquiriesByTypeTrendLatest,
  getNumberOfInquiriesByTypeTrend,
  updateNumberOfInquiriesByTypeTrend,
} from "../../../api";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "../../../store";
import { setNumberOfInquiriesByTypeTrend } from "../../../store/slices/AskWIUT/NumberOfInquiriesByTypeTrendSlice";
import { formatDate } from "../../../utils/custom";

const NumberOfInquiriesByTypeTrendTable = () => {
  const dispatch = useDispatch();
  const { numberOfInquiriesByTypeTrend } = useSelector(
    (state: RootState) => state.numberOfInquiriesByTypeTrendReducer
  );
  ...
  const handleCreateNumberOfInquiriesByTypeTrend = async () => {
    if (
      !statsName ||
      !statsValue1 ||
      !statsValue2 ||
      ...
      return;
    try {
      await createNumberOfInquiriesByTypeTrend({
        name: statsName,
        value1: statsValue1,
        value2: statsValue2,
        value3: statsValue3,
      ...
      setStatsValue2(null);
      setStatsValue3(null);
      handleGetNumberOfInquiriesByTypeTrend(filteredMonth);
    } catch (error) {
      console.log("createNumberOfInquiriesByTypeTrend error:", error);
    }
  };

  const handleGetNumberOfInquiriesByTypeTrend = async (
    filteredMonth: string = ""
  ) => {
    try {
      let res = await getNumberOfInquiriesByTypeTrend(filteredMonth);
      dispatch(setNumberOfInquiriesByTypeTrend(res.data));
    } catch (error) {
      console.log("getNumberOfInquiriesByTypeTrend error:", error);
    }
  };

  const handleGetLastReports = async () => {
    try {
      const res = await getNumberOfInquiriesByTypeTrendLatest();
      if (res.data && res.data.length > 0)
        return setFilteredMonth(res.data[0].calculated_date);
      setFilteredMonth("");
    } catch (error) {
      console.log("error getNumberOfInquiriesByTypeTrendLatest: ", error);
    }
  };
  ...
  const handleDeleteNumberOfInquiriesByTypeTrend = async (id: number) => {
    try {
      await deleteNumberOfInquiriesByTypeTrend(id);
      handleGetNumberOfInquiriesByTypeTrend(filteredMonth);
    } catch (error) {
      console.log("deleteNumberOfInquiriesByTypeTrend error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    if (
      ...
      return;
    try {
      await updateNumberOfInquiriesByTypeTrend({
        id: updateStatsId,
      ...
      console.log("updateNumberOfInquiriesByTypeTrend error:", error);
    }
  };

  const handleUpdateClick = (x: NumberOfInquiriesByTypeTrendData) => {
    ...
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!numberOfInquiriesByTypeTrend &&
          !!(numberOfInquiriesByTypeTrend.length > 0) &&
          numberOfInquiriesByTypeTrend.map((x, idx) => (
            <tr key={"numberOfInquiriesByTypeTrend-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteNumberOfInquiriesByTypeTrend(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateNumberOfInquiriesByTypeTrend}
            >
...
export default NumberOfInquiriesByTypeTrendTable;
