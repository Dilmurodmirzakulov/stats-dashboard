...
import {
  FrequentInquiriesCenterData,
  createFrequentInquiriesCenter,
  deleteFrequentInquiriesCenter,
  getFrequentInquiriesCenterLatest,
  getFrequentInquiriesCenter,
  updateFrequentInquiriesCenter,
} from "../../../api";
...
const FrequentInquiriesCenterTable = () => {
  ...
  const handleCreateFrequentInquiriesCenter = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createFrequentInquiriesCenter({
        name: statsName,
        value: statsValue,
      ...
      console.log("createFrequentInquiriesCenter error:", error);
    }
  };
  ...
  const handleGetFrequentInquiriesCenter = async (
    ...
    try {
      ...
      console.log("getFrequentInquiriesCenter error:", error);
    }
  };
  ...
  const handleGetLastReports = async () => {
    try {
      ...
      console.log("error getFrequentInquiriesCenterLatest: ", error);
    }
  };
  ...
  const handleDeleteFrequentInquiriesCenter = async (id: number) => {
    try {
      await deleteFrequentInquiriesCenter(id);
      handleGetFrequentInquiriesCenter(filteredMonth);
    } catch (error) {
      console.log("deleteFrequentInquiriesCenter error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    if (
      ...
      return;
    try {
      await updateFrequentInquiriesCenter({
        id: updateStatsId,
      ...
      console.log("updateFrequentInquiriesCenter error:", error);
    }
  };

  const handleUpdateClick = (x: FrequentInquiriesCenterData) => {
    ...
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!frequentInquiriesCenter &&
          !!(frequentInquiriesCenter.length > 0) &&
          frequentInquiriesCenter.map((x, idx) => (
            <tr key={"frequentInquiriesCenter-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteFrequentInquiriesCenter(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateFrequentInquiriesCenter}
            >
...
export default FrequentInquiriesCenterTable;
