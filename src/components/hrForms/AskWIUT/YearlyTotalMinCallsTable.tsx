...
import {
  YearlyTotalMinCallsData,
  createYearlyTotalMinCalls,
  deleteYearlyTotalMinCalls,
  getYearlyTotalMinCallsLatest,
  getYearlyTotalMinCalls,
  updateYearlyTotalMinCalls,
} from "../../../api";
...
const YearlyTotalMinCallsTable = () => {
  ...
  const handleCreateYearlyTotalMinCalls = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createYearlyTotalMinCalls({
        name: statsName,
        value: statsValue,
      ...
      console.log("createYearlyTotalMinCalls error:", error);
    }
  };
  ...
  const handleGetYearlyTotalMinCalls = async (filteredMonth: string = "") => {
    try {
      ...
      console.log("getYearlyTotalMinCalls error:", error);
    }
  };
  ...
  const handleDeleteYearlyTotalMinCalls = async (id: number) => {
    try {
      await deleteYearlyTotalMinCalls(id);
      handleGetYearlyTotalMinCalls(filteredMonth);
    } catch (error) {
      console.log("deleteYearlyTotalMinCalls error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    if (
      ...
      return;
    try {
      await updateYearlyTotalMinCalls({
        id: updateStatsId,
      ...
      console.log("updateYearlyTotalMinCalls error:", error);
    }
  };

  const handleUpdateClick = (x: YearlyTotalMinCallsData) => {
  ...
  return (
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!yearlyTotalMinCalls &&
          !!(yearlyTotalMinCalls.length > 0) &&
          yearlyTotalMinCalls.map((x, idx) => (
            <tr key={"yearlyTotalMinCalls-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteYearlyTotalMinCalls(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateYearlyTotalMinCalls}
            >
...
export default YearlyTotalMinCallsTable;
