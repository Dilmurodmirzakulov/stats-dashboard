...
import {
  HourlyCallsCenterData,
  createHourlyCallsCenter,
  deleteHourlyCallsCenter,
  getHourlyCallsCenterLatest,
  getHourlyCallsCenter,
  updateHourlyCallsCenter,
} from "../../../api";
...
const HourlyCallsCenterTable = () => {
  ...
  const handleCreateHourlyCallsCenter = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createHourlyCallsCenter({
        name: statsName,
        value: statsValue,
      ...
      console.log("createHourlyCallsCenter error:", error);
    }
  };
  ...
  const handleGetHourlyCallsCenter = async (filteredMonth: string = "") => {
    try {
      ...
      console.log("getHourlyCallsCenter error:", error);
    }
  };
  ...
  const handleDeleteHourlyCallsCenter = async (id: number) => {
    try {
      await deleteHourlyCallsCenter(id);
      handleGetHourlyCallsCenter(filteredMonth);
    } catch (error) {
      console.log("deleteHourlyCallsCenter error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    ...
    try {
      ...
      console.log("updateHourlyCallsCenter error:", error);
    }
  };

  const handleUpdateClick = (x: HourlyCallsCenterData) => {
  ...
  return (
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!hourlyCallsCenter &&
          !!(hourlyCallsCenter.length > 0) &&
          hourlyCallsCenter.map((x, idx) => (
            <tr key={"hourlyCallsCenter-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteHourlyCallsCenter(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateHourlyCallsCenter}
            >
...
export default HourlyCallsCenterTable;
