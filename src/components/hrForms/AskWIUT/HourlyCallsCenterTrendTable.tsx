...
import {
  HourlyCallsCenterTrendData,
  createHourlyCallsCenterTrend,
  deleteHourlyCallsCenterTrend,
  getHourlyCallsCenterTrendLatest,
  getHourlyCallsCenterTrend,
  updateHourlyCallsCenterTrend,
} from "../../../api";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "../../../store";
import { setHourlyCallsCenterTrend } from "../../../store/slices/AskWIUT/HourlyCallsCenterTrendSlice";
import { formatDate } from "../../../utils/custom";

const HourlyCallsCenterTrendTable = () => {
  const dispatch = useDispatch();
  const { hourlyCallsCenterTrend } = useSelector(
    (state: RootState) => state.hourlyCallsCenterTrendReducer
  );
  ...
  const handleCreateHourlyCallsCenterTrend = async () => {
    if (
      !statsName ||
      !statsValue1 ||
      !statsValue2 ||
      ...
      return;
    try {
      await createHourlyCallsCenterTrend({
        name: statsName,
        value1: statsValue1,
        value2: statsValue2,
        value3: statsValue3,
      ...
      setStatsValue2(null);
      setStatsValue3(null);
      handleGetHourlyCallsCenterTrend(filteredMonth);
    } catch (error) {
      console.log("createHourlyCallsCenterTrend error:", error);
    }
  };

  const handleGetHourlyCallsCenterTrend = async (
    filteredMonth: string = ""
  ) => {
    try {
      let res = await getHourlyCallsCenterTrend(filteredMonth);
      dispatch(setHourlyCallsCenterTrend(res.data));
    } catch (error) {
      console.log("getHourlyCallsCenterTrend error:", error);
    }
  };

  const handleGetLastReports = async () => {
    try {
      const res = await getHourlyCallsCenterTrendLatest();
      if (res.data && res.data.length > 0)
        return setFilteredMonth(res.data[0].calculated_date);
      setFilteredMonth("");
    } catch (error) {
      console.log("error getHourlyCallsCenterTrendLatest: ", error);
    }
  };
  ...
  const handleDeleteHourlyCallsCenterTrend = async (id: number) => {
    try {
      await deleteHourlyCallsCenterTrend(id);
      handleGetHourlyCallsCenterTrend(filteredMonth);
    } catch (error) {
      console.log("deleteHourlyCallsCenterTrend error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    if (
      ...
      return;
    try {
      await updateHourlyCallsCenterTrend({
        id: updateStatsId,
      ...
      console.log("updateHourlyCallsCenterTrend error:", error);
    }
  };

  const handleUpdateClick = (x: HourlyCallsCenterTrendData) => {
    ...
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!hourlyCallsCenterTrend &&
          !!(hourlyCallsCenterTrend.length > 0) &&
          hourlyCallsCenterTrend.map((x, idx) => (
            <tr key={"hourlyCallsCenterTrend-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteHourlyCallsCenterTrend(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateHourlyCallsCenterTrend}
            >
...
export default HourlyCallsCenterTrendTable;
