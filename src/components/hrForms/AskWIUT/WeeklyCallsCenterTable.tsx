...
import {
  WeeklyCallsCenterData,
  createWeeklyCallsCenter,
  deleteWeeklyCallsCenter,
  getWeeklyCallsCenterLatest,
  getWeeklyCallsCenter,
  updateWeeklyCallsCenter,
} from "../../../api";
...
const WeeklyCallsCenterTable = () => {
  ...
  const handleCreateWeeklyCallsCenter = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createWeeklyCallsCenter({
        name: statsName,
        value: statsValue,
      ...
      console.log("createWeeklyCallsCenter error:", error);
    }
  };
  ...
  const handleGetWeeklyCallsCenter = async (filteredMonth: string = "") => {
    try {
      ...
      console.log("getWeeklyCallsCenter error:", error);
    }
  };
  ...
  const handleDeleteWeeklyCallsCenter = async (id: number) => {
    try {
      await deleteWeeklyCallsCenter(id);
      handleGetWeeklyCallsCenter(filteredMonth);
    } catch (error) {
      console.log("deleteWeeklyCallsCenter error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    ...
    try {
      ...
      console.log("updateWeeklyCallsCenter error:", error);
    }
  };

  const handleUpdateClick = (x: WeeklyCallsCenterData) => {
  ...
  return (
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!weeklyCallsCenter &&
          !!(weeklyCallsCenter.length > 0) &&
          weeklyCallsCenter.map((x, idx) => (
            <tr key={"weeklyCallsCenter-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteWeeklyCallsCenter(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateWeeklyCallsCenter}
            >
...
export default WeeklyCallsCenterTable;
