...
import {
  StaffNationalityData,
  createStaffNationality,
  deleteStaffNationality,
  getStaffNationality,
  getStaffNationalityLatest,
...
const StaffNationalityTable = () => {
  ...
  const handleCreateStaffNationality = async () => {
    if (!statsName || !statsValue || !statsDate) return;
    try {
      await createStaffNationality({
        name: statsName,
        value: statsValue,
      ...
      console.log("createStaffNationality error:", error);
    }
  };
  ...
  const handleDeleteStaffNationality = async (id: number) => {
    try {
      await deleteStaffNationality(id);
      handleGetStaffNationality(filteredMonth);
    } catch (error) {
      console.log("deleteStaffNationality error:", error);
    }
  };
  ...
  const handleUpdateStats = async () => {
    ...
    try {
      ...
      console.log("updateStaffNationality error:", error);
    }
  };

  const handleUpdateClick = (x: StaffNationalityData) => {
    ...
    <Table striped hover responsive className="mb-3">
      ...
      <tbody>
        {!!staffNationality &&
          ...
          staffNationality.map((x, idx) => (
            <tr key={"staffNationality-id-" + x.id}>
              <td>{idx + 1}</td>
              <td>
                {!(updateStatsId == x.id) ? (
                  <span>{x.name}</span>
              ...
              <td className="text-end">
                ...
                <button
                  className="btn p-1"
                  onClick={() => handleDeleteStaffNationality(x.id)}
                >
        ...
        <tr>
          ...
          <td className="text-end">
            <button
              ...
              onClick={handleCreateStaffNationality}
            >
...
export default StaffNationalityTable;
