...
import { getInterviewsByCategory } from "../../../api";
import { setInterviewsByCategory } from "../../../store/slices/Recruitment/interviewsByCategorySlice";
import { getInterviewsByCategoryLatest } from "../../../api";

const InterviewsByCategory: FC = () => {
  ...
  const handleGetInterviewsByCategory = async (filteredMonth: string = "") => {
    try {
      ...
      console.log("error getInterviewsByCategory: ", error);
    }
  };
  ...
  useEffect(() => {
    if (!interviewsByCategory) return;
    setData(
      interviewsByCategory.map((x) => ({
        name: x.name,
        value: x.value,
      }))
    );
  ...
  return (
    <>
      <div className="d-md-flex align-items-center justify-content-between mb-3">
        <h4>Interviews conducted by category</h4>
        <input
...
export default InterviewsByCategory;
